...
from lunar_api.ioc import DependencyContainer, ServiceToken
...
class _TestImplementation(_TestService):
...
class AnotherService(ABC):
    @abstractmethod
    def get_name(self) -> str: ...


class AnotherImplementation(AnotherService):
...
@pytest.fixture
def test_token():
    return ServiceToken[_TestService](_TestService)
...
def test_register_service(container, test_token):
    container.register(test_token, _TestImplementation, "")
    service = container.get(test_token)
    assert isinstance(service, _TestImplementation)
    assert service.get_value() == "test"


def test_register_service_with_name(container, test_token):
    container.register(test_token, _TestImplementation, name="test_service")
...
def test_register_duplicate_service(container, test_token):
    container.register(test_token, _TestImplementation, "")
    with pytest.raises(
        ValueError, match="Service of type _TestService is already registered"
    ):
        container.register(test_token, _TestImplementation, "")
...
def test_register_duplicate_name(container, test_token, another_token):
    container.register(test_token, _TestImplementation, name="test_service")
    with pytest.raises(
        ValueError, match="Service with name 'test_service' is already registered"
    ):
...
def test_register_factory(container, test_token):
    def create_service():
        return _TestImplementation("factory_created")
...
def test_register_duplicate_factory(container, test_token):
    def create_service1():
        return _TestImplementation("first")

    def create_service2():
        return _TestImplementation("second")

    container.register_factory(test_token, create_service1, "")
    with pytest.raises(
        ValueError,
        match="Factory or service for type _TestService is already registered",
    ):
        container.register_factory(test_token, create_service2, "")


def test_get_nonexistent_service(container, test_token):
    with pytest.raises(KeyError, match="Service of type _TestService not found"):
        container.get(test_token)


def test_get_nonexistent_attribute(container):
    with pytest.raises(
        AttributeError, match="No service found for attribute 'nonexistent'"
    ):
...
def test_context_manager_cleans_up(container, test_token):
    with DependencyContainer() as ctx_container:
        ctx_container.register(test_token, _TestImplementation, "test")
        assert isinstance(ctx_container.get(test_token), _TestImplementation)
...
def test_context_manager_nested_containers(test_token):
    with DependencyContainer() as outer:
        outer.register(test_token, _TestImplementation, "outer")

        with DependencyContainer() as inner:
            inner.register(test_token, _TestImplementation, "test")
...
def test_context_manager_preserves_exceptions(test_token):
    with pytest.raises(ValueError, match="test exception"):
        with DependencyContainer() as container:
            container.register(test_token, _TestImplementation, "test")
            raise ValueError("test exception")

    with pytest.raises(KeyError):
        container.get(test_token)  # type: ignore
...
class ConfigImplementation(ConfigService):
    def __init__(self):
        self._config = {"api_key": "test_key", "endpoint": "test_endpoint"}

    def get_value(self, key: str) -> str:
        return self._config[key]


class DependentService(ABC):
    @abstractmethod
    def get_config_value(self, key: str) -> str: ...


class DependentImplementation(DependentService):
...
def test_register_with_dependency(container, config_token, dependent_token):
    ...
    container.register(
        dependent_token, DependentImplementation, "", config=config_token
    )
...
def test_missing_dependency(container, dependent_token):
    container.register(
        dependent_token,
        DependentImplementation,
        "",
        config=ServiceToken[ConfigService](ConfigService),
    )
...
def test_circular_dependency(container):
    ...
    class ImplementationA(ServiceA):
    ...
    class ImplementationB(ServiceB):
...
def test_multiple_dependencies(container):
    ...
    class ImplementationC(ServiceC):
